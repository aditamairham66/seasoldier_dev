    ...
    <script>
        WebFont.load({
            ...
            custom: {
                ...
                urls: ['{{ asset("vendor/backend/azzara") }}/css/fonts.css']
            },
            active: function () {
...
<div class="wrapper wrapper-login">
    <div class="container container-login animated fadeIn">
        <h3 class="text-center">
            <a href="{{url('/')}}">
                <img
                    title='{!!(Session::get('appname') == 'CRUDBooster')?"<b>CRUD</b>Booster":CRUDBooster::getSetting('appname')!!}'
                    src='{{ CRUDBooster::getSetting("logo")?asset(CRUDBooster::getSetting('logo')):asset('img/logo/logo-hitam.png') }}'
        ...
        <p class='login-box-msg text-center'>{{cbLang("login_message")}}</p>
        <form autocomplete='off' action="{{ route('postLogin') }}" method="post">
            <input type="hidden" name="_token" value="{{ csrf_token() }}"/>

            <div class="login-form">
                <div class="form-group form-floating-label">
                    <input autocomplete='off' type="text" class="form-control input-border-bottom" name='email'
                           required>
                    <label for="email" class="placeholder">Email</label>
                </div>
                <div class="form-group form-floating-label">
                    <input autocomplete='off' type="password" class="form-control input-border-bottom" name='password'
                           required>
                ...
                <div class="form-action mb-0">
                    <button type="submit"
                            class="btn btn-primary btn-rounded btn-login">{{cbLang("button_sign_in")}}</button>
                </div>
            </div>
        </form>
    </div>
