...
class NotificationsController extends CBController
{
    ...
    public function getLatestJson()
    {
        ...
        $rows = DB::table('cms_notifications')->where('id_cms_users', 0)->orWhere('id_cms_users', CRUDBooster::myId())->orderby('id', 'desc')->where('is_read', 0)->take(5);
        ...
        $rows = $rows->get();
        foreach ($rows as $row) {
            $row->created_at = CRUDBooster::timeAgo($row->created_at);
        }
