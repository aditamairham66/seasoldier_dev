<?php namespace App\Http\Controllers;
	...
	class AdminProfileTeamController extends \crocodicstudio\crudbooster\controllers\CBController {

	    public function colorHighlight($value)
        {
            switch ($value) {
                case 'Yes':
                    $color = 'success';
	    ...
	    public function cbInit() {
			...
			# OLD START FORM
			//$this->form = [];
			//$this->form[] = ["label"=>"Highlight","name"=>"highlight","type"=>"text","required"=>TRUE,"validation"=>"required|min:1|max:255"];
	    ...
	    */
	    public function hook_before_add(&$postdata) {
	        //Your code here
            $latest_sorting = DB::table('profile_team')
                ->max('sort');
	    ...
	    */
        ...
        public function getMoveUp($id)
        {
            ...
            if ($find->sort == 1) {
                return CRUDBooster::redirect(CRUDBooster::adminPath('profile-team'), 'Profile Team adalah profile team dengan urutan teratas', 'danger');
            } elseif ($find->sort > 1) {
            ...
            return CRUDBooster::redirect(CRUDBooster::adminPath('profile-team'), 'Berhasil merubah urutan profile team', 'success');
        }
        ...
        public function getMoveDown($id)
        {
            ...
            $above = ProfileTeam::findBy('sort', $find->sort + 1);
            if ($above->id == '') {
                return CRUDBooster::redirect(CRUDBooster::adminPath('profile-team'), 'Profile Team adalah profile team dengan urutan terbawah', 'danger');
            } elseif ($above->id != '') {
                $above->sort = $find->sort;
                $above->save();
